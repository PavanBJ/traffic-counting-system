...
##out = cv2.VideoWriter('traffic_count.mp4',cv2.VideoWriter_fourcc('M','J','P','G'), 10, (frame_width,frame_height))
##
##while(True):
##  ret, frame = cap.read()
##
...
# Video source
inputFile = loc+'/inputs/625_201708311636.mp4'
...
##            ############updating value to firebase###########################
            ...
            print (vehi_LHS_string + vehi_RHS_string + ',' + time_hhmmss + ',' + date_mmddyyyy + ',' + location)

            #insert record
            data = {'date':date_mmddyyyy,'time':time_hhmmss,'RH LANE':vehi_RHS_string,'LH LANE':vehi_LHS_string}
            result = requests.post(firebase_url + '/' + location + '/VEHICLECOUNT.json', data=json.dumps(data))
